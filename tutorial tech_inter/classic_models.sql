...
phone varchar(50) DEFAULT '+56992698111',
...
SHOW TABLES;
DESCRIBE offices;
INSERT INTO offices (officeCode, city, phone, addressLine1, country, postalCode, territory) values ( 'aaa', 'Bengaluru', '+45684732', '1 MG Road', 'India', '560001', 'Asia');
SELECT officeCode, city from offices;
SELECT * FROM  offices; 
INSERT INTO  offices VALUES ( 'aab', 'Santiago', '+56987655','Av Estadio 45',' esquina ','Santiago', 'Chile', '1234', 'South America'); 
INSERT INTO offices (officeCode, city, phone, addressLine1, country, postalCode, territory) values 
( 'aad', 'Bengaluru', '+45684732', '67 EG Road', 'India', '560001', 'Asia'),
( 'aae', 'Bengaluru', '+45684733', ' 1 FG Road', 'India', '560001', 'Asia'),
( 'aaf', 'Bengaluru', '+45684733', '54 MH Road', 'India', '560001', 'Asia'),
( 'aag', 'Bengaluru', '+45684532', '34 ELVIS Road', 'India', '560001', 'Asia'),
( 'aah', 'Bengaluru', '+45684632', '67 MAR Road', 'India', '560001', 'Asia'),
( 'aai', 'Bengaluru', '+45684742', '4 MF Road', 'India', '560001', 'Asia');
...
INSERT INTO employees VALUES ( '1003','Jeldres', 'Diane','x5800','djeldres@classicmodelcars.com', 'aaa',NULL,'countryManager');
...
INSERT INTO `Offices` (`officeCode`,`city`,`phone`,`addressLine1`,`addressLine2`,`state`,`country`,`postalCode`,`territory` ) VALUES ('6','Sydney','+61 2 9264 2451','5-11 Wentworth Avenue','Floor #2','NSW','Australia','2010','APAC');
...
# List the costumers in USA with a credit limit higher than $1000
...
#1) List the employee codes for sales representatives of customers in Spain, France and Italy
SELECT SalesRepEmployeeNumber FROM Customers WHERE country = ("Spain", "France", "Italy"); # do not work
SELECT SalesRepEmployeeNumber FROM Customers WHERE country = "Spain" and country = "France" and country=  "Italy"; #do not work
SELECT SalesRepEmployeeNumber FROM Customers WHERE country IN ("Spain", "France", "Italy");

#2) How to obtain unique values from a column.
# DISTINCT syntax; SELECT DISTINCT column_name FROM table_name; 
SELECT DISTINCT SalesRepEmployeeNumber FROM Customers WHERE country IN ("Spain", "France", "Italy");
 
 #3) Make another query to list the names and email addresses of those employees. 
# SalesRepEmployeeNumber = 1370, 1337, 1702, 1401
SELECT firstName, lastName, email FROM employees WHERE employeeNumber IN (1370, 1337, 1702, 1401);
SELECT firstName, lastName, email FROM employees WHERE employeeNumber = ´SalesRepEmployeeNumber´ FROM Customers WHERE country IN ("Spain", "France", "Italy");

# Change the job title "Sales Rep" to "Sales Representative"
#1) visualize the table 
SELECT * FROM employees; 
#2) UPDATE syyntax; UPDATE table_name SET column_name = "new_change" CONDITION 
UPDATE employees SET jobTitle ="Sales Representative" WHERE jobTitle ="Sales Rep";
 
 # Show a list of customers and the country with "TOYS" in their name 
 #1) LIKE syntax; SELECT column1,column2,... FROM table_name WHERE columnN LIKE pattern;
...
# Order the customers names by the countries ( alphabetical) 
SELECT customerName, country FROM customers order by country; 
# like Head in python 
SELECT customerName, country, creditLimit FROM customers WHERE creditLimit >10000 LIMIT 10; 

# List the 5 most expensive products from the Planes product line 
SELECT productLine, buyPrice, quantityInStock FROM products WHERE productLine= "planes" ORDER BY buyPrice desc LIMIT 5;

#Identify the products that are about to run out of stock ( quantity in stock < 100) 
SELECT productLine, quantityInStock FROM products WHERE quantityInStock <100; 

# List 10 products in the "Motorcycles" category with the lowest buy price and more than 1000 units in stock. 
SELECT productLine,buyPrice, quantityInStock FROM products WHERE productLine = "Motorcycles" AND quantityInStock >1000 ORDER BY buyPrice desc  LIMIT 10; 
...
SHOW TABLES; 
DESCRIBE products;
# what is the average percentage markup of MSRP on buyPrice
SELECT * FROM products;
...
SELECT customerNumber FROM payments WHERE paymentDate <"2004-10-28";
...
# the detais of customers are in customers table. so we need to chain the result of the previous query with the customer table. 
SELECT * FROM customers WHERE customerNumber in (SELECT DISTINCT customerNumber FROM payments WHERE paymentDate <"2004-10-28"); 

#Retrieve details of all the customers in the USA who have made payments between April 1st 2003 and March 31st 2004
SELECT * FROM customers WHERE country = "USA";
SELECT * FROM payments WHERE paymentDate >= "2003-04-01" and paymentDate <= "2004-03-31" ;
# chaining data 
SELECT * FROM customers WHERE country = "USA"  AND customerNumber IN (SELECT customerNumber FROM payments WHERE paymentDate >= "2003-04-01" and paymentDate <= "2004-03-31" );

# find the total number of payment made each customer before october 28, 2004
...
select * from products; 
SELECT productCode, count(quantityOrdered) as TotalUnitsSold FROM orderdetails GROUP BY productCode; 

# find the total number of payments and total payment amount for each customer for payments made before october 28, 2004. 
SELECT customerNumber,COUNT(*) as TotalNumberOfPayment, SUM(amount) as TotalPayment FROM payments WHERE paymentDate < "2004-10-28" GROUP by customerNumber;

#show the minimun and maximun and average payment value for each customer. 
SELECT customerNumber,COUNT(*) as TotalNumberOfPayment, SUM(amount) as TotalPayment, MIN(amount) as MinPayment , MAX(amount) as MaxPayment, AVG(amount) as AveragePayment FROM payments WHERE paymentDate < "2004-10-28" GROUP by customerNumber;

#Retrieve the customer number for 10 customers who made highest total payment before October 28, 2004
...
#Retrieve the customer number for 10 customers who made after 10 highest total payment before October 28, 2004
...
SELECT customerName, CONCAT(UCASE(contactFirstName), " ", UCASE(contactLastName)) AS contactFullName,phone FROM customers WHERE country = "USA" ORDER BY customerName;

#Display a paginated list of customers (sorted by customer name), with a country code column. The country code are the first 3 letters in the country name in lower case.
SELECT customerName, LCASE(SUBSTRING(country, 1,3)) AS CountryCode FROM customers ORDER BY customerName;

# Display the list of the 5 most expensive products in the Motorcycle products line with their price (MSRP) rounded to dollars
SELECT productCode, productName, ROUND(MSRP) as RoundPrice FROM products WHERE productLine ="Motorcycles" ORDER BY MSRP DESC LIMIT 5; 

# Display the product code, product name, buy price, sale price and profit margin percentage ((MSRP-buyPrice)*100/buyPrice) for the 10 products with the highest profit margen
#Round the profit margin to 2 decimals. 
SELECT productCode, productName, buyPrice, MSRP as salePrice, ROUND((MSRP-buyPrice)*100/buyPrice, 2) as profitMarginPercentage FROM products ORDER BY profitMarginPercentage DESC LIMIT 10;

#For the above query, format the amount properly with a dollar symbol and comma separation, and also show the month as a string. 
SELECT CONCAT ("$",FORMAT(amount , 2)) as AmountInDollar, YEAR(paymentDate) as payYear, MONTHNAME(paymentDate) as payMonth FROM payments GROUP BY paymentDate; 

#show the 10 most recent payments with customer details (name and phone)
SELECT payments.*,customers.customerName ,customers.phone FROM payments JOIN customers ON payments.customerNumber = customers.customerNumber ORDER BY paymentDate DESC LIMIT 10;

#who reports to whom, 
SELECT E.employeeNumber, CONCAT(E.firstName, " ", E.lastName) as EmployeeName, CONCAT(M.firstName, " ", M.lastName) AS managerName FROM employees E LEFT JOIN employees M on E.reportsTo = M.employeeNumber;

# CREATE  view FOR usa CUSTOMERS
CREATE VIEW American_Customers as SELECT * FROM  customers WHERE country ="USA"; 
