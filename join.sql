...
primary key(id),
unique index (nombre),
foreign key(proveedorid) references proveedor(id) # dato que proviene de otra tabla: proveerdor
);
describe productos;
...
#Estructura del Join para saber el nombre del proveedor sin que traiga los id
select p.nombre, p.descripcion, p.precio, pro.nombre from productos as p # alias  p = producto pro= proveedor 
...
create table clientes(
  nombre varchar(50),
  mail varchar(70),
  tarjetacredito blob, #no se puede ver
  primary key (nombre)
);
...
insert into clientes 
  ...
  select cast(aes_decrypt(tarjetacredito, 'xyz123') as char) from clientes; # codigo para descencriptar
